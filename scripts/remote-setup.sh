...
HOOK_SCRIPT="$SCRIPT_DIR/dev-island-hooks.py"
REMOTE_BIN_DIR=".local/bin"
REMOTE_HOOK_PATH="\$HOME/$REMOTE_BIN_DIR/dev-island-hooks.py"
...
REMOTE="$1"
LOCAL_UID=$(id -u)
SOCKET_NAME="dev-island-${LOCAL_UID}.sock"

echo "==> Deploying dev-island-hooks.py to $REMOTE ..."
ssh "$REMOTE" "mkdir -p ~/$REMOTE_BIN_DIR"
scp "$HOOK_SCRIPT" "$REMOTE:~/$REMOTE_BIN_DIR/dev-island-hooks.py"
ssh "$REMOTE" "chmod +x ~/$REMOTE_BIN_DIR/dev-island-hooks.py"
...
# forwarded socket created by the local Dev Island app.  The heredoc is
# unquoted so that $SOCKET_NAME is expanded.
HOOK_CMD="DEV_ISLAND_SOCKET_PATH=/tmp/$SOCKET_NAME python3 ~/.local/bin/dev-island-hooks.py --source claude"
