...
final class TerminalJumpServiceTests: XCTestCase {
    ...
    func testGhosttyJumpScriptFallsBackToWorkingDirectoryAndTitle() {
        ...
        XCTAssertTrue(script.contains("(working directory of aTerminal as text) is \"/Users/wangruobing/Personal/dev-island\""))
        XCTAssertTrue(script.contains("(name of aTerminal as text) contains \"codex ~/p/dev-island\""))
        XCTAssertTrue(script.contains("if \"\" is \"\" then"))
    }

    func testGhosttyJumpIntegrationMatchesFocusedTerminalForLiveSurfaces() throws {
        guard ProcessInfo.processInfo.environment["DEV_ISLAND_RUN_GHOSTTY_JUMP_INTEGRATION"] == "1" else {
            throw XCTSkip("Set DEV_ISLAND_RUN_GHOSTTY_JUMP_INTEGRATION=1 to run live Ghostty jump verification.")
        }
    ...
    func testCursorJumpActivatesRunningAppWithoutWorkspaceReuse() throws {
        ...
        let result = try service.jump(
            to: JumpTarget(
                ...
                workingDirectory: "/Users/test/dev-island"
            )
        )
    ...
    func testTraeCNJumpFallsBackToWorkspaceViaTraeCLI() throws {
        ...
        XCTAssertEqual(processInvocations.values.first?.1, ["-r", "/Users/test/dev-island"])
    }
}
