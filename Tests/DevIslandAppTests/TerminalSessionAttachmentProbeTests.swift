...
struct TerminalSessionAttachmentProbeTests {
    ...
    func ghosttyRehomesFromTitleWorkspaceWhenJumpTargetDirectoryIsWrong() {
        ...
        let rehomed = AgentSession(
            ...
            jumpTarget: JumpTarget(
                terminalApp: "Ghostty",
                workspaceName: "dev-island",
                paneTitle: "codex ~/tmp/dev-island",
                workingDirectory: "/tmp/dev-island",
                terminalSessionID: "ghostty-1"
            )
        )
        ...
        let resolutions = probe.sessionResolutions(
            ...
            ghosttyAvailability: .available(
                [
                    .init(sessionID: "ghostty-1", workingDirectory: "/tmp/dev-island", title: "codex ~/tmp/dev-island"),
    ...
    func unavailableGhosttyProbeStillAttachesActiveClaudeSession() {
        ...
        let updates = probe.attachmentStates(
            ...
            terminalAvailability: .available([] as [TerminalSessionAttachmentProbe.TerminalTabSnapshot], appIsRunning: false),
            activeProcesses: [
                .init(tool: .claudeCode, sessionID: nil, workingDirectory: "/tmp/dev-island", terminalTTY: "/dev/ttys002"),
            ],
            now: now
        )
    ...
    func ghosttyExactMatchPrefersRunningClaudeSessionOverNewerCompletedSession() {
        ...
        let runningSession = AgentSession(
            ...
            jumpTarget: JumpTarget(
                terminalApp: "Ghostty",
                workspaceName: "dev-island",
                paneTitle: "claude --dangerously ~/p/dev-island",
                workingDirectory: "/tmp/dev-island",
        ...
        let updates = probe.attachmentStates(
            ...
            ghosttyAvailability: .available(
                [.init(sessionID: "ghostty-claude", workingDirectory: "/tmp/dev-island", title: "claude --dangerously ~/p/dev-island")],
                appIsRunning: true
            ),
            terminalAvailability: .available([] as [TerminalSessionAttachmentProbe.TerminalTabSnapshot], appIsRunning: false),
            now: now
    ...
    func unknownTerminalSessionRehomesToGhosttyFromWorkingDirectory() {
        ...
        let resolutions = probe.sessionResolutions(
            for: [session],
            ghosttyAvailability: .available(
                [.init(sessionID: "ghostty-1", workingDirectory: "/tmp/dev-island", title: "claude ~/tmp/dev-island")],
                appIsRunning: true
            ),
            terminalAvailability: .available([] as [TerminalSessionAttachmentProbe.TerminalTabSnapshot], appIsRunning: false),
            now: now
        ...
        #expect(resolutions["claude-session"]?.correctedJumpTarget?.workingDirectory == "/tmp/dev-island")
    }
    ...
    func activeCodexSessionRehomesToRemainingGhosttySnapshot() {
        ...
        let resolutions = probe.sessionResolutions(
            ...
            ghosttyAvailability: .available(
                [
                    .init(sessionID: "ghostty-1", workingDirectory: "/tmp/dev-island", title: "codex ~/tmp/dev-island"),
                    .init(sessionID: "ghostty-2", workingDirectory: "/tmp/dev-island", title: "codex ~/tmp/dev-island"),
                ],
                appIsRunning: true
            ),
            terminalAvailability: .available([] as [TerminalSessionAttachmentProbe.TerminalTabSnapshot], appIsRunning: false),
            activeProcesses: [
                .init(tool: .codex, sessionID: "primary", workingDirectory: "/tmp/dev-island", terminalTTY: "/dev/ttys000"),
                .init(tool: .codex, sessionID: "active-rehomed", workingDirectory: "/tmp/dev-island", terminalTTY: "/dev/ttys001"),
            ],
            now: now
        )
    ...
    func claudeSessionIDPrefixInGhosttyTitleBeatsSameDirectoryCodexSession() {
        ...
        let resolutions = probe.sessionResolutions(
            ...
            ghosttyAvailability: .available(
                [
                    .init(sessionID: "ghostty-codex", workingDirectory: "/tmp/dev-island", title: "codex ~/tmp/dev-island"),
                    .init(sessionID: "ghostty-claude", workingDirectory: "/tmp/dev-island", title: "dev-island · hi · e45d5e87-66d0-4f"),
                ],
                appIsRunning: true
            ),
            terminalAvailability: .available([] as [TerminalSessionAttachmentProbe.TerminalTabSnapshot], appIsRunning: false),
            activeProcesses: [
                .init(tool: .codex, sessionID: "codex-session", workingDirectory: "/tmp/dev-island", terminalTTY: "/dev/ttys000"),
                .init(tool: .claudeCode, sessionID: nil, workingDirectory: "/tmp/dev-island", terminalTTY: "/dev/ttys002"),
            ],
            now: now
        )
        ...
        #expect(resolutions["e45d5e87-66d0-4f67-8399-6ebc02f3d453"]?.correctedJumpTarget?.paneTitle == "dev-island · hi · e45d5e87-66d0-4f")
    }
    ...
    func claudePrefixClaimOverridesMisbindingRecordedGhosttySessionID() {
        ...
        let misboundCodexSession = ghosttySession(
            ...
            phase: .running,
            terminalSessionID: "ghostty-claude",
            paneTitle: "dev-island · hi · e45d5e87-66d0-4f",
            workingDirectory: "/tmp/dev-island"
        )
        let claudeSession = AgentSession(
            id: "e45d5e87-66d0-4f67-8399-6ebc02f3d453",
        ...
        let resolutions = probe.sessionResolutions(
            ...
            terminalAvailability: .available([] as [TerminalSessionAttachmentProbe.TerminalTabSnapshot], appIsRunning: false),
            activeProcesses: [
                .init(tool: .codex, sessionID: "misbound-codex", workingDirectory: "/tmp/dev-island", terminalTTY: "/dev/ttys001"),
                .init(tool: .claudeCode, sessionID: nil, workingDirectory: "/tmp/dev-island", terminalTTY: "/dev/ttys002"),
            ],
            now: now
        )
    ...
    func activeCodexSessionWithoutJumpTargetCanAttachFromProcessWorkingDirectory() {
        ...
        #expect(resolutions["active-no-jump-target"]?.correctedJumpTarget?.workingDirectory == "/tmp/dev-island")
    }
    ...
    func ghosttySessionListRegressionOnlyKeepsLiveLookingTabsAttached() {
        ...
        let sessions = [
            ghosttySession(
                ...
                paneTitle: "codex ~/p/dev-island",
                workingDirectory: "/Users/wangruobing/personal/dev-island",
                workspaceName: "dev-island"
            ),
            ghosttySession(
    ...
    func ghosttyPlainShellTitleDoesNotFallbackAttachOlderSessionByDirectory() {
        ...
        let updates = probe.attachmentStates(
            for: [session],
            ghosttyAvailability: .available(
                [.init(sessionID: "ghostty-shell", workingDirectory: "/tmp/dev-island", title: "~/p/dev-island")],
                appIsRunning: true
            ),
            terminalAvailability: .available([] as [TerminalSessionAttachmentProbe.TerminalTabSnapshot], appIsRunning: false),
            now: now
