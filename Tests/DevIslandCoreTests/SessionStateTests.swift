...
struct SessionStateTests {
    ...
    func actionableStateResolvedClearsWaitingForApproval() {
        ...
        state.apply(
            .actionableStateResolved(
                ActionableStateResolved(
                    ...
                    summary: "Approval was handled outside Dev Island.",
                    timestamp: startedAt.addingTimeInterval(10)
                )
            )
        )
        ...
        #expect(state.session(id: "claude-approval")?.summary == "Approval was handled outside Dev Island.")
    }
    ...
    func codexHookInstallerMergesManagedGroupsWithoutDroppingUnrelatedHooks() throws {
        ...
        let mutation = try CodexHookInstaller.installHooksJSON(
            ...
            hookCommand: "'/tmp/DevIslandHooks'"
        )
        ...
        let managedStopHook = stopGroups?
            .compactMap { $0["hooks"] as? [[String: Any]] }
            .flatMap { $0 }
            .first(where: { $0["command"] as? String == "'/tmp/DevIslandHooks'" })

        #expect(stopCommands.contains("/usr/bin/true"))
        #expect(stopCommands.contains("'/tmp/DevIslandHooks'"))
    ...
    @Test
    func codexHookInstallerInstallReplacesLegacyDevIslandCommands() throws {
        let existing = """
        {
          "hooks": {
            "PreToolUse": [
              {
                ...
                "hooks": [
                  {
                    "type": "command",
                    "command": "'/Users/test/.dev-island/bin/dev-island-bridge' --source codex"
                  },
                  {
                    "type": "command",
            ...
            "Stop": [
              {
                "hooks": [
                  ...
                  {
                    "type": "command",
                    "command": "'/tmp/old-debug/DevIslandHooks'"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true"
        ...
        let mutation = try CodexHookInstaller.installHooksJSON(
            existingData: existing,
            hookCommand: "'/tmp/new-release/DevIslandHooks'"
        )
        ...
        #expect(permissionCommands == ["'/tmp/new-release/DevIslandHooks'"])
        #expect(hooks?["PostToolUse"] == nil)
        #expect(stopCommands.contains("/usr/bin/true"))
        #expect(stopCommands.contains("'/tmp/new-release/DevIslandHooks'"))
        #expect(!stopCommands.contains("'/Users/test/.dev-island/bin/dev-island-bridge' --source codex"))
        #expect(!stopCommands.contains("'/tmp/old-debug/DevIslandHooks'"))
    }
    ...
    func codexHookInstallerUninstallRemovesOnlyManagedHooks() throws {
            ...
            "Stop": [
              {
                "hooks": [
                  {
                    "type": "command",
                    "command": "'/tmp/DevIslandHooks'",
                    "statusMessage": "Managed by Dev Island"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true",
        ...
        let mutation = try CodexHookInstaller.uninstallHooksJSON(
            existingData: existing,
            managedCommand: "'/tmp/DevIslandHooks'"
        )
    ...
    func codexHookPayloadCarriesSessionLocatorIntoJumpTarget() {
        let payload = CodexHookPayload(
            ...
            terminalTitle: "codex ~/P/dev-island",
            transcriptPath: nil
        )
        ...
        #expect(jumpTarget.paneTitle == "codex ~/P/dev-island")
    }

    @Test
    func codexHookInstallationManagerRoundTripsInstallAndUninstall() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("dev-island-tests-\(UUID().uuidString)", isDirectory: true)
        ...
        let managedHooksBinaryURL = rootURL
            ...
            .appendingPathComponent("DevIslandHooks")
