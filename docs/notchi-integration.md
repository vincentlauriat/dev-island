...
[`notchi`](https://github.com/sk-ruban/notchi) is a strong adjacent reference for Dev Island:
...
Dev Island should learn from that execution without collapsing into a Claude-specific mascot app.
...
That is a useful product pattern for Dev Island:

1. ambient awareness in the notch
2. actionable detail on expand
3. local transport and setup automation underneath

## Where Dev Island Should Differ

Notchi is optimized for Claude Code presence and delight. Dev Island should stay optimized for control.

Dev Island should keep these distinctions:
...
Dev Island already has the beginning of the Codex side:

- [Sources/DevIslandCore/CodexHooks.swift](/Users/wangruobing/Personal/dev-island/Sources/DevIslandCore/CodexHooks.swift)
- [Sources/DevIslandHooks/main.swift](/Users/wangruobing/Personal/dev-island/Sources/DevIslandHooks/main.swift)

The next step is to add a native installer that:

- detects Codex config presence
- installs the `DevIslandHooks` executable or a thin wrapper script
...
Dev Island should do a control-oriented version of that:
...
Dev Island can adopt that idea in a more neutral form:
...
Notchi demonstrates that a narrow first integration is enough to ship. Dev Island should still keep its bridge generic so Claude Code or other agents can be added later as adapters.

The current shared model and bridge transport already point in the right direction:

- [Sources/DevIslandCore/AgentEvent.swift](/Users/wangruobing/Personal/dev-island/Sources/DevIslandCore/AgentEvent.swift)
- [Sources/DevIslandCore/BridgeTransport.swift](/Users/wangruobing/Personal/dev-island/Sources/DevIslandCore/BridgeTransport.swift)
- [Sources/DevIslandCore/SessionState.swift](/Users/wangruobing/Personal/dev-island/Sources/DevIslandCore/SessionState.swift)
...
That would capture the highest-leverage lessons from Notchi while staying aligned with Dev Island's stated product direction.
...
Those features may help Notchi's product identity, but they are not on Dev Island's current critical path.
