...
final class ConnectionManager: ObservableObject {
    private static let logger = Logger(subsystem: "app.devisland.mobile", category: "ConnectionManager")
    ...
    enum ConnectionState {
        ...
        var displayText: String {
            switch self {
            case .disconnected: return NSLocalizedString("connection.state.disconnected", comment: "")
            case .discovering: return NSLocalizedString("connection.state.discovering", comment: "")
            case .paired: return NSLocalizedString("connection.state.paired", comment: "")
            case .connected: return NSLocalizedString("connection.state.connected", comment: "")
            }
        }
    ...
    @Published var notifyPermissions: Bool {
        didSet { UserDefaults.standard.set(notifyPermissions, forKey: "devisland.notify.permissions") }
    }
    @Published var notifyQuestions: Bool {
        didSet { UserDefaults.standard.set(notifyQuestions, forKey: "devisland.notify.questions") }
    }
    @Published var notifyCompletions: Bool {
        didSet { UserDefaults.standard.set(notifyCompletions, forKey: "devisland.notify.completions") }
    }
    @Published var silentCompletions: Bool {
        didSet { UserDefaults.standard.set(silentCompletions, forKey: "devisland.notify.silentCompletions") }
    }
    ...
    private var savedToken: String? {
        get { UserDefaults.standard.string(forKey: "devisland.token") }
        set { UserDefaults.standard.set(newValue, forKey: "devisland.token") }
    }
    private var savedMacName: String? {
        get { UserDefaults.standard.string(forKey: "devisland.macName") }
        set { UserDefaults.standard.set(newValue, forKey: "devisland.macName") }
    }
    var pairedAt: Date? {
        get {
            let interval = UserDefaults.standard.double(forKey: "devisland.pairedAt")
            return interval > 0 ? Date(timeIntervalSince1970: interval) : nil
        }
        set {
            if let date = newValue {
                UserDefaults.standard.set(date.timeIntervalSince1970, forKey: "devisland.pairedAt")
            } else {
                UserDefaults.standard.removeObject(forKey: "devisland.pairedAt")
            }
        }
    }
    ...
    init() {
        ...
        self.notifyPermissions = defaults.object(forKey: "devisland.notify.permissions") as? Bool ?? true
        self.notifyQuestions = defaults.object(forKey: "devisland.notify.questions") as? Bool ?? true
        self.notifyCompletions = defaults.object(forKey: "devisland.notify.completions") as? Bool ?? true
        self.silentCompletions = defaults.object(forKey: "devisland.notify.silentCompletions") as? Bool ?? false
    ...
    private func startNetworkMonitor() {
            ...
            Task { @MainActor [weak self] in
                ...
                if wasSatisfied && !isSatisfied {
                    ...
                    if self.state == .connected {
                        self.state = .paired
                        self.connectionError = NSLocalizedString("connection.error.wifiLost", comment: "")
                    }
        ...
        monitor.start(queue: DispatchQueue(label: "app.devisland.mobile.network"))
    ...
    private func handleSSEDisconnect() {
        ...
        state = .paired
        connectionError = NSLocalizedString("connection.error.reconnecting", comment: "")
    ...
    private func handleTokenExpired() {
        ...
        savedToken = nil
        state = .disconnected
        connectionError = NSLocalizedString("pairing.error.expired", comment: "")
    }
}
...
enum PairingError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidCode: return NSLocalizedString("pairing.error.invalidCode", comment: "")
        case .codeExpired: return NSLocalizedString("pairing.error.codeExpired", comment: "")
        case .invalidResponse: return NSLocalizedString("pairing.error.invalidResponse", comment: "")
        case let .serverError(code):
            return String(format: NSLocalizedString("pairing.error.serverError", comment: "%lld is the HTTP status code"), code)
        case .resolutionFailed: return NSLocalizedString("pairing.error.resolutionFailed", comment: "")
        case .notConnected: return NSLocalizedString("pairing.error.notConnected", comment: "")
        case .tokenExpired: return NSLocalizedString("pairing.error.expired", comment: "")
        }
    }
}
