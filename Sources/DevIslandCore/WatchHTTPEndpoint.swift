...
///
/// Uses `NWListener` for TCP + Bonjour advertising of `_devisland._tcp`.
...
public final class WatchHTTPEndpoint: @unchecked Sendable {
    private static let logger = Logger(subsystem: "app.devisland", category: "WatchHTTPEndpoint")
    private static let serviceType = "_devisland._tcp"
    private static let pairingCodeLength = 4
    private static let pairingCodeExpiry: TimeInterval = 120 // 2 minutes

    private let queue = DispatchQueue(label: "app.devisland.watch.http", qos: .userInitiated)
