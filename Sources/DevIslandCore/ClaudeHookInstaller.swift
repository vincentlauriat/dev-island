...
public struct ClaudeHookInstallerManifest: Equatable, Codable, Sendable {
    public static let fileName = "dev-island-claude-hooks-install.json"
...
public enum ClaudeHookInstaller {
    ...
    private static func isLegacyDevIslandHookCommand(_ command: String) -> Bool {
        ...
        if (normalized.contains("devislandhooks") || normalized.contains("vibeislandhooks")) && normalized.contains("--source claude") {
            return true
        }
