...
public struct GeminiHookInstallerManifest: Equatable, Codable, Sendable {
    public static let fileName = "dev-island-gemini-hooks-install.json"
...
public enum GeminiHookInstaller {
    ...
    private static func isManagedGroup(_ group: [String: Any], managedCommand: String?) -> Bool {
        ...
        return hooks.contains { hook in
            ...
            return isDevIslandGeminiHookCommand(command)
        }
    }

    private static func isDevIslandGeminiHookCommand(_ command: String) -> Bool {
        let normalized = command.lowercased()
        return (normalized.contains("devislandhooks") || normalized.contains("vibeislandhooks"))
