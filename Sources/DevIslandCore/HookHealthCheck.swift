...
public struct HookHealthReport: Equatable, Sendable {
    ...
    public enum Issue: Equatable, Sendable, CustomStringConvertible {
        ...
        /// Other hooks detected alongside Dev Island hooks (informational).
...
public enum HookHealthCheck {
    ...
    public static func checkClaude(
        ...
        // 3. Check manifest
        if fileManager.fileExists(atPath: settingsPath),
           hasDevIslandHooks(in: settingsURL, fileManager: fileManager) {
    ...
    public static func checkCodex(
        ...
        // 3. Check manifest
        if fileManager.fileExists(atPath: hooksPath),
           hasDevIslandHooks(in: hooksURL, fileManager: fileManager) {
    ...
    public static func checkOpenCode(
        ...
        let pluginFileURL = pluginsDir.appendingPathComponent("dev-island.js")
        ...
        for (_, eventValue) in hooks {
            ...
            for group in groups {
                ...
                for hook in hookEntries {
                    ...
                    // Only check Dev Island / Vibe Island commands
                    let normalized = command.lowercased()
                    guard normalized.contains("devislandhooks") || normalized.contains("vibeislandhooks")
                        || normalized.contains("dev-island") || normalized.contains("vibe-island") else {
                        continue
                    }
    ...
    /// Finds third-party (non-Dev Island) hook command names for display.
        ...
        for (_, eventValue) in hooks {
            ...
            for group in groups {
                ...
                for hook in hookEntries {
                    ...
                    // Skip Dev Island / Vibe Island hooks
                    if normalized.contains("devislandhooks") || normalized.contains("vibeislandhooks")
                        || normalized.contains("dev-island") || normalized.contains("vibe-island") {
                        continue
                    }
    ...
    /// Checks whether any Dev Island managed hooks exist in a config file.
    private static func hasDevIslandHooks(in url: URL, fileManager: FileManager) -> Bool {
