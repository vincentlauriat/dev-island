// Dev Island plugin for OpenCode
// Bridges OpenCode events to the Dev Island desktop app via Unix socket.
// Install: copy to ~/.config/opencode/plugins/dev-island.js
import { connect } from "net";
import { appendFileSync } from "fs";
import { homedir } from "os";

const DEBUG_LOG = "/tmp/dev-island-opencode-debug.log";
function debugLog(msg) {
  try { appendFileSync(DEBUG_LOG, `[${new Date().toISOString()}] ${msg}\n`); } catch {}
}

const SOCKET_PATH =
  process.env.DEV_ISLAND_SOCKET_PATH ||
  `${process.env.HOME || homedir()}/Library/Application Support/DevIsland/bridge.sock`;
  ...
  return {
    "event": async ({ event }) => {
      try {
        ...
        // Fail open: if Dev Island is unavailable, don't block OpenCode
      }
    },

    "shell.env": async (input, output) => {
      output.env.DEV_ISLAND_ACTIVE = "1";
