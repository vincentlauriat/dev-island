...
@MainActor
final class DevIslandAppDelegate: NSObject, NSApplicationDelegate {
    let model = AppModel()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ProcessInfo.processInfo.disableAutomaticTermination(
            "Dev Island should remain active while monitoring local agent sessions."
        )
        ...
        DispatchQueue.main.async { [self] in
            ...
            DevIslandAppDelegate.hideAllAppWindows()
...
struct DevIslandApp: App {
    @NSApplicationDelegateAdaptor(DevIslandAppDelegate.self)
    private var appDelegate

    @Environment(\.openWindow) private var openWindow

    var body: some Scene {
        Window("Dev Island Settings", id: "settings") {
