...
final class AppModel {
    ...
    func showSettings() {
        ...
        if let window = NSApp.windows.first(where: { $0.title == "Dev Island Settings" }) {
    ...
    func approvePermission(for sessionID: String, action: ApprovalAction) {
        ...
        switch action {
        case .deny:
            resolution = .deny(message: "Permission denied in Dev Island.", interrupt: false)
    ...
    private func permissionResolution(for approved: Bool) -> PermissionResolution {
        ...
        return .deny(message: "Permission denied in Dev Island.", interrupt: false)
    }
