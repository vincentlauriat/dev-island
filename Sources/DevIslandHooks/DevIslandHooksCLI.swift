...
struct DevIslandHooksCLI {
    ...
    static func main() {
        do {
            // Allow wrappers to delegate one child process away from Dev Island without changing global hook installation.
            // 允许外部控制器只让当前子进程跳过 Dev Island hook，不影响全局安装状态。
    ...
    private static func logStderr(_ message: String) {
        guard let data = "[DevIslandHooks] \(message)\n".data(using: .utf8) else { return }
